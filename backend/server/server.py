...
async def output(json_data: dict, request: Request, response: Response):
    try:
        ...
        cache_service_config_key = ("serviceIp",clientApiKey)
        cached_service_data = cache.get(cache_service_config_key)

        if cached_service_data:
            serviceIp = cached_service_data
        else:
            serviceIp = os.getenv("AIServicesIp")
            cache[cached_service_data] = serviceIp
        ...
        sericveApi = deploymentDetails["serviceApi"]
        serviceUrl = "http://"+serviceIp+":"+sericveApi
        
        json_data.update(deploymentDetails)
        json_data.pop("serviceApi")
        del deploymentDetails
        
        response = requests.post(serviceUrl, json = json_data)
